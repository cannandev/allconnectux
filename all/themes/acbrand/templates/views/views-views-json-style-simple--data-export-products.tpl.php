...
// print $rows['plans'][0]['key'];
// print $rows;

$new_rows = array();

foreach ($rows['plans'] as $key => $value) {
$price = explode('.', $rows['plans'][$key]['price']);

	array_push($new_rows, array(
		'key' => $rows['plans'][$key]['key'],
		'attributes' => '',
		'content' => [array(
			'name' =>  
				array(
				'type'=> 'text',
				'text'=> $rows['plans'][$key]['name'],
				),
			'category' =>  
				array(
				'type'=> 'array',	
				'text'=> explode(',', strtolower($rows['plans'][$key]['categories'])),
				),
			'description' =>  
				array(
				'type'=> 'text',
				'text'=> $rows['plans'][$key]['description'],
				),
			'priceData' =>  
				array(
				'type'=> 'price',
				'term'=> $rows['plans'][$key]['unit'],
				'normal' => array(
						'dollars' => $price[0],
						'cents' => $price[1],
						'price' => $rows['plans'][$key]['price'],
					)
				),
			'terms' =>  
				array(
				'type'=> 'text',
				'text'=> $rows['plans'][$key]['terms'],
				),
			'label' =>  
				array(
				'type'=> 'text',
				'text'=> $rows['plans'][$key]['label'],
				),
			'filter' =>  
				array(
				'type'=> 'text',
				'text'=> $rows['plans'][$key]['filter'],
				),
			'cta' =>  
				array(
				'type'=> 'text',
				'text'=> $rows['plans'][$key]['cta'],
				),
			'icons' =>  
				array(
				'type'=> 'array',
				'text'=> explode(',', $rows['plans'][$key]['icons']),
				),
			)				
		])
	);
